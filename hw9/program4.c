...
void *readData1(void *args){
    ...
    int sid;
    int prev1=-1,count=0,current=-1,result=-1;
    bool print=false;
    while((access("output1.txt",F_OK))==-1);
    started[0]=true;
    while(1){
...
void *readData2(void *args){
    ...
    bool print =false;
    while((access("output2.txt",F_OK))==-1 ||!started[0]);
    started[1]=true;
    while(1){
        if((sid=semget(SKey,1,0660))>-1){
            if((result=semctl(sid,0,GETVAL,NULL))!=1){
...
void *readData3(void *args){
    ...
    bool print=false;
    FILE *inpt,*outpt;
    while((access("output3.txt",F_OK))==-1 || !started[1] || !started[0]);
    started[2]=true;
    while(1){
        if((sid=semget(SKey,1,0660))>-1){
            if((result=semctl(sid,0,GETVAL,NULL))!=1){
